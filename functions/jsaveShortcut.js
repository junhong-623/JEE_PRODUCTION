...
function parseTngScreenshot(ocrText) {
  ...
  const type = amountMatch[2] === '-' ? 'expense' : 'income'
  if (type === 'expense' && !/支付/.test(text)) throw new Error('invalid-type')
  if (type === 'income' && !/(从钱包接收|接收转账)/.test(text)) throw new Error('invalid-type')
  if (fieldAfter(text, '状态') !== '成功') throw new Error('not-successful')
  ...
  let note = type === 'expense' ? fieldAfter(text, '商家') : fieldAfter(text, '接收转账')
  if (type === 'expense' && !note) {
    note = text.match(/支付\s*[-–—]\s*([^\n]+)/)?.[1]?.trim() || ''
  }
  if (!note || note.length > 120 || /^(款项详情|付款方式|日期\/时间|钱包参考号|状态|交易编号)$/.test(note)) {
    throw new Error('missing-party')
  }

  let sourceTransactionId = fieldAfter(text, '交易编号')
