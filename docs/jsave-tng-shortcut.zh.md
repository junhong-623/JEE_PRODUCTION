...
当前下载文件是 v3.6.6，尚未包含导入后删除截图的菜单。仓库中的 Cherri 源码已加入该菜单，待新版签名文件生成并在 iPhone 验证后才会替换下载文件。
...
截图本身不上传；发送到 JSave 的是 iPhone 提取的文字。详情页要求金额、交易日期、商家或转账人、成功状态和交易编号。扫码付款成功页要求 `已转账`、金额、`接收者` 和 `日期与时间`。详情页按 TNG 交易编号去重；成功页没有交易编号，按金额、时间和接收者生成去重标识。

## 4. 在 iPhone 添加截屏自动化

在 iOS 27，打开「快捷指令 → 自动化 → +」，搜索「截屏」，选择截图保存到「照片」时触发。Apple 的「截屏」触发器没有 App 条件，所以它适用于**所有 App**。若选择运行前确认，其他 App 截图也会收到询问；只在 TNG 使用时可忽略该询问。自动化需要在每台 iPhone 上设置一次，不能随下载的 `.shortcut` 一起安装。
...
如果希望只对 TNG 截图弹出询问，可在自动化中先对截图执行「从图像提取文字」，再用「如果」检查 TNG 交易页的文字特征，然后才显示「选择菜单：导入 JSave／略过」。例如详情页可检查 `交易类型`、`钱包参考号`；扫码付款成功页可检查 `已转账`、`接收者`、`日期与时间`。这种判断只能按画面内容筛选，不能证明截图一定来自 TNG；实际交易仍须在 JSave 的核对提醒中确认。

导入成功后如需删除截图，快捷指令应在接口返回 `created` 或 `duplicate` 后显示「保留照片／删除照片」，把**本次输入的照片**传给「删除照片」。不要用「获取最新截图」作为删除目标，避免误删另一次截图。删除可能再次要求照片权限；先用一张可丢弃的截图在 iPhone 测试。

## 限制

- 目前识别规则覆盖提供的两种 TNG 详情页（`-RM` 支付与 `+RM` 从钱包接收）和扫码付款成功页（`RM`、`已转账`、`接收者`）。其他 TNG 页面、OCR 漏字、失败或处理中交易会拒绝导入。
- 扫码付款成功页没有 TNG 交易编号。重复导入同一页会被识别，但若同一时间、同一金额付给同一人，系统也会当作重复；从成功页和详情页各导入一次则可能成为两笔，请只导入其中一种截图。
