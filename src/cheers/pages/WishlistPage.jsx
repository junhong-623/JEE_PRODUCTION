        ...
        {items.map(item => (
          <div key={item.productId} className="card p-4 flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              {(() => {
                const n = item.name
                const displayName = typeof n === 'object' ? (n?.[lang] || n?.zh || n?.en || '') : (n || '')
                return (
                  <Link to={`/products/${item.productId}`}>
                    <p className="font-medium text-cheers-dark-brown text-sm line-clamp-2 hover:text-cheers-brown">{displayName}</p>
                  </Link>
                )
              })()}
