...
export default function ServicesPage() {
  ...
  return (
    <>
      ...
      <section className="bg-[#f8f3f2] py-20 sm:py-28"><div className="mx-auto max-w-7xl px-6"><p className="font-mono text-[10px] uppercase tracking-[.3em] text-[#b66b81]">A COMPLETE WORKFLOW</p><h2 className="mt-4 max-w-2xl font-display text-5xl leading-tight">{zh ? '每一场直播，前后都有专业支持。' : 'Professional support around every live session.'}</h2><div className="mt-12 grid gap-5 md:grid-cols-3">{support.map(([title, copy], index) => <article key={title} className="border border-[#eadde0] bg-white p-8"><p className="font-mono text-[10px] text-[#b66b81]">0{index + 1}</p><h3 className="mt-7 font-display text-3xl">{title}</h3><p className="mt-4 text-sm leading-7 text-gray-500">{copy}</p></article>)}</div><Link to={path('/join')} className="mt-12 inline-flex rounded-full bg-[#24171c] px-7 py-3.5 text-sm font-semibold text-white">{zh ? '开始你的申请' : 'Start your application'} ↗</Link></div></section>
    </>
  )
}
