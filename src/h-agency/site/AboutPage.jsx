...
export default function AboutPage() {
  ...
  return (
    <>
      ...
      <section className="bg-white py-20 sm:py-28"><div className="mx-auto grid max-w-7xl gap-14 px-6 lg:grid-cols-[1.08fr_.92fr] lg:items-center lg:gap-20"><Reveal direction="left" className="relative"><img src="/hagency/editorial/about-team.webp" alt={zh ? '希望公会运营团队共同复盘主播内容' : 'ℋ Agency team reviewing creator content'} className="aspect-[3/2] w-full object-cover shadow-[0_30px_90px_rgba(65,30,42,.14)]" /><div className="absolute -bottom-6 right-6 flex items-center gap-3 bg-[#24171c] px-5 py-4 text-white shadow-xl"><img src="/hagency/logo.jpg" alt="" className="h-11 w-11 rounded-full object-cover" /><div><p className="font-display text-xl italic text-[#efced6]">ℋ Agency</p><p className="text-[8px] tracking-[.22em] text-white/40">希望公会</p></div></div></Reveal><Reveal direction="right"><p className="font-mono text-[10px] uppercase tracking-[.3em] text-[#b66b81]">OUR NAME · OUR INTENT</p><h2 className="mt-4 font-display text-5xl leading-tight">ℋ Agency<br /><span className="text-[#b66b81]">希望公会</span></h2><p className="mt-7 max-w-xl text-base leading-8 text-gray-500">{zh ? '“希望”不是一句空泛的口号。对我们来说，它是一套具体的工作：认识主播、找到方向、共同执行、持续复盘，让原本模糊的潜力逐渐成为清晰的个人品牌。' : '“Hope” is practical work: knowing the creator, finding a direction, executing together and learning continuously until potential becomes a clear personal brand.'}</p><div className="mt-9 grid grid-cols-3 border-y border-[#eadde0] py-5"><div><p className="font-display text-3xl">{Math.max(talents.length, 3)}+</p><p className="mt-1 text-[9px] uppercase tracking-[.16em] text-gray-400">{zh ? '展示主播' : 'Talent'}</p></div><div className="border-l border-[#eadde0] pl-5"><p className="font-display text-3xl">CN</p><p className="mt-1 text-[9px] uppercase tracking-[.16em] text-gray-400">{zh ? '中文市场' : 'Market'}</p></div><div className="border-l border-[#eadde0] pl-5"><p className="font-display text-3xl">360°</p><p className="mt-1 text-[9px] uppercase tracking-[.16em] text-gray-400">{zh ? '成长支持' : 'Support'}</p></div></div></Reveal></div></section>
      <section className="bg-[#f8f3f2] py-20 sm:py-28"><div className="mx-auto max-w-7xl px-6"><Reveal><p className="font-mono text-[10px] uppercase tracking-[.3em] text-[#b66b81]">WHAT WE BELIEVE</p><h2 className="mt-4 max-w-2xl font-display text-5xl leading-tight">{zh ? '专业，来自清楚的原则。' : 'Professional work begins with clear principles.'}</h2></Reveal><div className="mt-12 grid gap-5 md:grid-cols-3">{principles.map(([title, copy], index) => <Reveal key={title} delay={index * 90}><article className="h-full border border-[#eadde0] bg-white p-8"><span className="font-mono text-[10px] text-[#b66b81]">0{index + 1}</span><h3 className="mt-7 font-display text-3xl">{title}</h3><p className="mt-4 text-sm leading-7 text-gray-500">{copy}</p></article></Reveal>)}</div></div></section>
