...
export default function LoginPage() {
  const { loginEmail, loginGoogle, loginGoogleGIS, loginGuest, register } = useAuth();
  ...
  // Mount Google's official button on iOS PWA (GIS iframe flow — no popup/redirect)
  useEffect(() => {
    if (!iosPwa || !gisBtnRef.current) return;
    loginGoogleGIS(gisBtnRef.current).catch(err => {
      setError(err.message || "Google sign-in failed.");
    });
  }, [iosPwa]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleGuest = async () => {
    ...
    try {
      ...
      if (msg.includes("operation-not-allowed") || msg.includes("auth/operation-not-allowed")) {
        setError("Guest login is not enabled yet. Please sign in with email or Google.");
      } else {
        setError(msg.replace("Firebase: ", "").replace(/\(auth\/.*\)\.?/, "").trim() || "Guest sign-in failed.");
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <main className="login-page">
      ...
      <div className="login-card">
        ...
        {iosPwa ? (
          // GIS renders its own button here — works on iOS Safari PWA without popup/redirect
          <div
            ref={gisBtnRef}
            style={{
              width: "100%",
        ...
        <div className="guest-section">
          <div className="guest-section-label">{zh ? "无需注册？" : "No account?"}</div>
          <button
            className="btn btn-ghost btn-lg guest-btn"
            onClick={handleGuest}
            disabled={loading}
          >
            👀 {zh ? "以游客身份浏览" : "Continue as Guest"}
          </button>
          <div className="guest-hint">
            {zh
              ? <>游客可加入 1 个行程（只读）。<br />用示例码 <strong className="guest-code">{SAMPLE_TRIP_CODE}</strong> 查看示例行程。</>
              : <>Guest can join 1 trip (read-only).<br />Try sample code <strong className="guest-code">{SAMPLE_TRIP_CODE}</strong> to explore.</>
            }
          </div>
        </div>
      </div>
