...
export default function SummaryPage({ toast, reportLocked }) {
  const { activeTrip } = useTrip();
  const { user } = useAuth();
  const { tr, t } = useLang();
  const { showUpgrade } = useMember();
    ...
    <div>
      ...
      <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
        ...
        <button
          onClick={() => {
            if (reportLocked) {
              showUpgrade("Travel Report requires VIP or above. Upgrade to unlock.");
              return;
            }
          ...
          style={{
            ...
            color: reportLocked ? "var(--ink-muted)" : summaryTab === "report" ? "var(--terracotta)" : "var(--ink-muted)",
            fontWeight: summaryTab === "report" ? 700 : 500,
            fontSize: 13, cursor: "pointer", fontFamily: "var(--font-body)",
            opacity: reportLocked ? 0.55 : 1,
          }}>
          {reportLocked ? "🔒" : "📊"} {tr.tripReport || "旅途报告"}
        </button>
      </div>
