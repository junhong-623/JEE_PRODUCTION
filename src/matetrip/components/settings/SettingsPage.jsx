...
export default function SettingsPage({ toast }) {
  ...
  const { memberLevel, getLimits, showUpgrade, isGuest } = useMember();
  const zh = lang === "zh";
  ...
  return (
    <div>
      ...
      {/* Member Level */}
      <div className="card" style={{marginBottom:12, borderLeft:`3px solid ${levelInfo.color}`}}>
        <div className="section-title" style={{marginBottom:10}}>
          {zh ? "会员等级" : "Membership"}
        </div>
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
          <div style={{flex:1}}>
            <span style={{fontSize:18,fontWeight:800,color:levelInfo.color,fontFamily:"var(--font-display)"}}>
              {zh ? levelInfo.labelZh : levelInfo.label}
            </span>
          </div>
          {memberLevel === "vvip" ? (
            ...
            <button className="btn btn-primary" style={{fontSize:12,padding:"6px 14px"}}
              onClick={() => showUpgrade(zh ? "升级即可解锁更多功能" : lang === "ms" ? "Naik taraf untuk buka lebih banyak ciri" : "Upgrade to unlock more features")}>
              🚀 {zh ? "升级" : lang === "ms" ? "Naik Taraf" : "Upgrade"}
            </button>
          )}
        </div>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,fontSize:12}}>
          {[
            [zh?"行程上限":"Trip limit",      limits.tripLimit     ?? "∞"],
            [zh?"账单上限":"Receipt / trip",   limits.receiptLimit  ?? "∞"],
            [zh?"图片上限":"Images / trip",    limits.imageLimit    ?? "∞"],
            [zh?"日程上限":"Schedule / trip",  limits.scheduleLimit ?? "∞"],
            [zh?"旅行报告":"Travel report",    limits.report  ? "✓" : "✗"],
            [zh?"留言功能":"Chat",             limits.chat    ? "✓" : "✗"],
          ].map(([k,v]) => (
            <div key={k} style={{display:"flex",justifyContent:"space-between",
              background:"var(--sand)",borderRadius:6,padding:"5px 8px"}}>
              <span style={{color:"var(--ink-muted)"}}>{k}</span>
              <span style={{fontWeight:600,color:v==="✗"?"var(--ink-muted)":v==="✓"?"var(--success,#4ac882)":"var(--ink)"}}>{v}</span>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="card" style={{marginBottom:12}}>
        ...
        {isGuest ? (
          /* Guest — show register prompt instead of edit options */
          <div style={{background:"var(--sand)",borderRadius:10,padding:"12px 14px",marginBottom:8,
            display:"flex",flexDirection:"column",gap:8,textAlign:"center"}}>
            <div style={{fontSize:13,color:"var(--ink-muted)",lineHeight:1.6}}>
              {zh
                ? "游客账号无法修改个人信息。注册免费账号即可自定义用户名和密码。"
                : lang === "ms"
                  ? "Akaun tetamu tidak boleh mengedit profil. Daftar akaun percuma untuk tetapkan nama pengguna dan kata laluan."
                  : "Guest accounts cannot edit profile info. Register a free account to set your username and password."}
            </div>
            <a href="/matetrip"
              className="btn btn-primary"
              style={{textDecoration:"none",display:"block",textAlign:"center"}}>
              🔑 {zh ? "登录 / 注册" : lang === "ms" ? "Log Masuk / Daftar" : "Sign In / Register"}
            </a>
          </div>
        ) : (
          <>
