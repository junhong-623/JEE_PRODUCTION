...
const API = (import.meta.env.VITE_MATETRIP_API_BASE_URL || "http://localhost:3001").replace(/\/$/, "");
...
// ─── Main PlanPage ─────────────────────────────────────────────────────────────
export default function PlanPage({ toast, chatLocked, readOnly }) {
  ...
  // Reset to schedule if chat gets disabled or locked
  useEffect(() => {
    if ((!chatEnabled || chatLocked) && activeTab === "chat") setActiveTab("schedule");
  ...
  useEffect(() => {
    ...
    const blockTouch = (e) => {
      if (!e.target.closest(".receipt-panel")) e.preventDefault();
    };
  ...
  const handleDeleteEvent = async () => {
    ...
    setShowEventModal(false);
    setEditEvent(null);
    toast.show(tr.receiptDeleted || "Deleted");
  };

  const handleSendMessage = async () => {
    const text = msgText.trim();
    if (!text || sending) return;
    if (chatLocked || readOnly || isGuest) { toast.show(tr.errChatLocked, "error"); return; }
    setSending(true);
    setMsgText("");
  ...
  return (
    <div className="plan-page">
      ...
      <div className="plan-panel plan-tabs">
        ...
        <button
          className={`plan-tab ${activeTab === "chat" ? "active" : ""} ${chatLocked ? "nav-locked" : ""}`}
          onClick={() => {
            if (chatLocked) { showUpgrade("Chat requires VVIP plan. Upgrade to unlock."); return; }
            setActiveTab("chat"); markRead();
          }}>
          {chatLocked ? "🔒" : "💬"} {tr.chat}
          {!chatLocked && (() => {
                ...
                <div key={date} className="schedule-day">
                  ...
                  {isExpanded && items.map(item => (
                    <div key={item.id} className="event-card"
                      onClick={!readOnly && !isGuest ? () => { setEditEvent(item); setShowEventModal(true); } : undefined}
                      style={readOnly || isGuest ? {cursor:"default"} : {}}>
