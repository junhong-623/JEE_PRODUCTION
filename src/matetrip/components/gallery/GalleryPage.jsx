...
export default function GalleryPage({ toast, readOnly }) {
  ...
  // Compute limit state for render
  const renderLimits = getLimits(activeTrip?.limits || {});
  const maxPhotos = renderLimits.imageLimit ?? photosMaxPerTrip ?? 5;
  const atLimit = !readOnly && !isGuest && photos.length >= maxPhotos;
  ...
  const handleFileSelect = (e) => {
    ...
    if (!file) return;
    if (readOnly || isGuest) { toast.show(tr.errGuestNoWrite, "error"); e.target.value = ""; return; }
    // Member-level image limit (takes priority over admin photosMaxPerTrip)
    const limits = getLimits(activeTrip?.limits || {});
    const maxPhotos = limits.imageLimit ?? photosMaxPerTrip ?? 5;
    if (photos.length >= maxPhotos) {
      showUpgrade(`Image limit reached (${maxPhotos} / trip). Upgrade to upload more.`);
      e.target.value = "";
      return;
    }
  ...
  return (
    <div>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20 }}>
        ...
        {!readOnly && !isGuest && photosEnabled ? (
          atLimit ? (
            <button className="btn btn-primary" onClick={() => showUpgrade(tr.errTripLimitReached)}>
      ...
      {filteredPhotos.length === 0 && photos.length > 0 ? (
        ...
        <div className="empty-state">
          ...
          {!readOnly && !isGuest && photosEnabled && (
            atLimit ? (
              <button className="btn btn-primary" style={{ marginTop: 16 }} onClick={() => showUpgrade(tr.errTripLimitReached)}>
                🔒 {photos.length}/{maxPhotos} — {zh ? "升级解锁" : "Upgrade to add more"}
              </button>
            ) : (
