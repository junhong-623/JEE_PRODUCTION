...
export function TripProvider({ children }) {
  const { user } = useAuth();
  const { getLimits, isGuest, showUpgrade } = useMember();
  const { tr } = useLang();
  ...
  const createTrip = async (data) => {
    if (isGuest) throw new Error(tr.errGuestNoCreate);

    const limits = getLimits();
    if (limits.tripLimit !== null && trips.length >= limits.tripLimit) {
      showUpgrade(tr.errTripLimitReached);
      throw new Error(tr.errTripLimitReached);
    }
  ...
  const joinTrip = async (code) => {
    ...
    const snap = await getDocs(q);
    if (snap.empty) throw new Error(tr.errTripNotFound);
    const tripDoc = snap.docs[0];
    const tripData = { id: tripDoc.id, ...tripDoc.data() };
    if (!isGuest && tripData.createdBy === user.uid) throw new Error(tr.errAlreadyCreator);
    if ((tripData.memberIds || []).includes(user.uid)) throw new Error(tr.errAlreadyJoined);
    await updateDoc(doc(db, "trips", tripDoc.id), {
      memberIds: arrayUnion(user.uid),
    });
    if (!isGuest) await autoCreateCompanion(tripDoc.id, user.uid, db);
    return tripData;
  };
